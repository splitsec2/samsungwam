"""Config-flow tests for the samsungwam integration.

Covers the Bronze "config-flow-test-coverage" surface plus the two
behavioural fixes on the `user` step:

  * 1a happy path            -> coverage (passes on both branches)
  * 1b connect-error path    -> re-show form (all-fixes) vs abort (master)
  * 1c ssdp + already-config -> coverage (passes on both branches)
  * 1d custom port field     -> port stored (all-fixes) vs no port field (master)
...
# ---------------------------------------------------------------------------
# 1a. user happy path -> creates a config entry (coverage; both branches)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1b. user error path -> re-show form with errors["base"] == "cannot_connect"
#     (all-fixes). On master the flow ABORTS instead, so this FAILS there.
# ---------------------------------------------------------------------------
async def test_user_connect_error_reshows_form(hass: HomeAssistant) -> None:
    ...
    # all-fixes: form is re-shown with a recoverable error.
...
# ---------------------------------------------------------------------------
# 1c. ssdp discovery -> confirm form (coverage; both branches)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1c. already-configured abort (coverage; both branches)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1d. user step accepts a custom port and stores it (all-fixes).
